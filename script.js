...
`;
  ...
  gsap.to(imageSeq, {
    ...
    onUpdate: function () {
      render();
      const displayedFrame = Math.round(
        (imageSeq.frame / (frameCount - 1)) * 100
      );
      counter.textContent = displayedFrame + "%";
    },
  });
...
function canvas3() {
  const canvas = document.querySelector("#page11-canvas>canvas");
  const context = canvas.getContext("2d");
